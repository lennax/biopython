...
class Site(object):
    ...
    def _parse_info(self, info_str, pair_sep=";", key_sep="=", list_sep=","):
        """PRIVATE: parse info string into key, val pairs; return dict"""
        info_dict = {}
        # magic
        for pair in info_str.split(pair_sep):
            if key_sep in pair:
                key, val = pair.split(key_sep, 1)
                if list_sep in val:
                    val = val.split(list_sep)
                info_dict[key] = val
            else:
                key = pair
                info_dict[key] = True
